...
const DashboardScreen: React.FC<Props> = () => {
  ...
  const navigation =
    useNavigation<
      BottomTabScreenProps<TabParamList, "DashboardTab">["navigation"]
    >();
  ...
  const loadAccounts = async (): Promise<void> => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        (data as Record<string, unknown>).shared_accounts &&
        Array.isArray((data as Record<string, unknown>).shared_accounts)
      ) {
        accountList = (data as Record<string, unknown>)
          .shared_accounts as Account[];
      } else if ((data as Record<string, unknown>)[0]) {
        accountList = Object.values(
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      <ScreenHeader
        ...
        subtitle={`Bienvenue${user?.firstName || user?.lastName ? ` ${[user?.firstName, user?.lastName].filter(Boolean).join(" ")}` : ""} 👋`}
        rightContent={
          <CustomButton
            label="Déconnexion"
          ...
          >
            {accounts.map((account) => {
              const progress = Math.min(
                (account.currentAmount / account.targetAmount) * 100,
                100,
              );
              return (
                <CustomCard
                ...
                >
                  ...
                  <View style={{ marginVertical: 12 }}>
                    ...
                    >
                      {account.currentAmount.toFixed(0)} /{" "}
                      {account.targetAmount.toFixed(0)} {account.currency}
                    </Text>
                  </View>
